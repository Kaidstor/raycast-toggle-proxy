import { ActionPanel, Action, Icon, List, Form, LocalStorage, showToast, Toast, useNavigation } from "@raycast/api";
...
export default function SSHTunnelManager() {
  ...
  return (
    <List isLoading={loading} searchBarPlaceholder="Управление SSH-туннелями">
      {error && <List.EmptyView icon={Icon.ExclamationMark} title="Ошибка" description={error} />}
        ...
        <List.Section title="Недавние подключения (быстрый запуск)">
          {recentTunnels.map((rt, idx) => (
            <List.Item
              ...
              actions={
                <ActionPanel>
                  <Action icon={Icon.Play} title="Быстрый Запуск" onAction={() => quickLaunch(rt)} />
                </ActionPanel>
              }
            />
          ))}
      ...
      {!error && (
        <List.Item
          ...
          actions={
            <ActionPanel>
              <Action title="Открыть Форму" icon={Icon.Plus} onAction={openNewTunnelForm} />
            </ActionPanel>
          }
        />
      )}
...
function NewTunnelForm(props: {
  ...
  return (
    ...
    >
      ...
      <Form.Dropdown id="sshHost" title="Хост из ~/.ssh/config" value={selectedHost} onChange={setSelectedHost}>
        {hosts.map((h) => (
